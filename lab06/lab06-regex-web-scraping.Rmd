...
institution <- stringr::str_extract_all(
  publications_txt,
  "University\\s+of\\s+[[:alpha:]\\s]+ | [[:alpha:]\\s]+\\s+Institute\\s+of\\s+[[:alpha:]\\s]+"
  ) 
...
31 publications don't have an abstract.
Now, the title
...
```md
View [here](https://ghcdn.rawgit.org/SmokeTrails/JSC370-labs/master/lab06/lab06-regex-web-scraping.html)
```
