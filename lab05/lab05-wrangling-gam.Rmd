...
``` {.sh, message=FALSE, warning=FALSE}
# Step 1
cd ~/Documents
mkdir JSC370-labs
cd JSC370-labs
...
```{r, message=FALSE, warning=FALSE}
met <- data.table::fread("../jsc370-2022/labs/lab03/met_all.gz")
met <- merge(x = met, y = stations, all.x = T, all.y = F, by.x = "USAFID", 
             by.y = "USAF")
```
...
```{r, message=FALSE, warning=FALSE}
station_ave <- met[, .(temp = mean(temp, na.rm = T), wind.sp = 
                         mean(wind.sp, na.rm = T), atm.press = 
                         mean(atm.press, na.rm = T)), by = .(USAFID, STATE)]
medians <- station_ave[, .(temp_50 = quantile(temp, probs = 0.5, na.rm = T),
...
```{r, message=FALSE, warning=FALSE}
mid_point <- met[, .(lon_50 = quantile(lon, probs = 0.5, na.rm = T),
                     lat_50 = quantile(lat, probs = 0.5, na.rm = T)),
                 by = STATE]
...
The gam model is definitely better than the linear model. We can see from the scatterplot as well as the low r-squared value of 0.0169 that the linear model is a poor choice to determine the association between temperature and wind speed. On the other hand, the gam model fits the data well (based on the scatterplot), and it also has a high r-squared value of 0.67. However, one of the problems with the gam model is that it may be overfitting and "memorizing" the data.

```{r, message=FALSE, warning=FALSE}
station_ave[, temp_50 := quantile(temp, probs = 0.5, na.rm = T), by = STATE]
station_ave[, wind.sp_50 := quantile(wind.sp, probs = 0.5, na.rm = T), by = STATE]        
library(ggplot2)
...
-   Paste the link to the github repo you created here:                 
    https://github.com/SmokeTrails/JSC370-labs 
